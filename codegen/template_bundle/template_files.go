...
func Test{{$testFixture.HandleID | title}}{{$testFixture.TestName | title}}OKResponse(t *testing.T) {
	...
	fake{{$clientCallFixture.ClientMethod | title}} := func(w http.ResponseWriter, r *http.Request) {
		...
		{{ if $clientCallFixture.ClientResponse -}}
		clientResponseString := {{printf "` + "`" + `%s` + "`" + `" $clientCallFixture.ClientResponse.String}}
		{{else}}
	...
	{{ if $testFixture.EndpointRequest -}}
	endpointRequestString := {{printf "` + "`" + `%s` + "`" + `" $testFixture.EndpointRequest.String}}
	{{else}}
...
func endpoint_testTmpl() (*asset, error) {
	...
	info := bindataFileInfo{name: "endpoint_test.tmpl", size: 3851, mode: os.FileMode(420), modTime: time.Unix(1, 0)}
	a := &asset{bytes: bytes, info: info}
	return a, nil
}
...
func Test{{title $testFixture.HandleID}}{{title $testFixture.TestName}}OKResponse(t *testing.T) {
	...
	{{end -}}
	{{if $responseType -}}
		assert.Equal(t, []byte(` + "`" + `{{$testFixture.EndpointResponse}}` + "`" + `), data)
	{{end -}}
}
...
func endpoint_test_tchannel_clientTmpl() (*asset, error) {
	...
	info := bindataFileInfo{name: "endpoint_test_tchannel_client.tmpl", size: 4560, mode: os.FileMode(420), modTime: time.Unix(1, 0)}
	a := &asset{bytes: bytes, info: info}
	return a, nil
}
